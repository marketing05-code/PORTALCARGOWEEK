// CARGOWEEK · Misión Entrega Final
...
const CW_HEADERS = ["Fecha", "Nombre", "Sucursal", "Fase", "Misiones", "Errores", "Tiempo", "Puntaje", "Juego"];
...
function doPost(e) {
  try {
    ...
    const record = {
      ...
      "Fase": data.fase || ("Fase "+(data.faseActiva||1)),
      "Misiones": data.misiones ?? 0,
      "Errores": data.errores ?? 0,
      "Tiempo": data.tiempo || "",
      "Puntaje": data.puntaje ?? 0,
      "Juego": data.juego || "Misión Entrega Final"
    };
...
function getRanking_(fase) {
  ...
  for(let r=1;r<vals.length;r++){
    ...
    const valor=Number(cell_(row,idx,"puntaje"))||0; const detalle=valor+" pts";
